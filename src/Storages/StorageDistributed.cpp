    ...
    const size_t /*num_streams*/)
{
    ...
    if (local_context->getSettingsRef().allow_experimental_analyzer)
    {
        ...
        /** For distributed tables we do not need constans in header, since we don't send them to remote servers.
          * Moreover, contants can break some functions like `hostName` that are constants only for local queries.
          */
